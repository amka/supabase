...
Running Realtime will generate a lot of redundant database logs. You can mute these logs by setting your database's `log_min_messages` configuration to `fatal`. However, you might miss important log messages, such as database errors, by doing so.
